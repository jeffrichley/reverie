# Justfile — single quality-command surface for reverie workspace.
#
...
pkg-src := "packages/reverie/src"
pkg-tests := "packages/reverie/tests"
...
fix:
    ...
    uv run --no-sync ruff format packages/reverie

# Lint
lint:
    uv run --no-sync ruff check packages/reverie

# Type-check
typecheck:
    uv run --no-sync mypy packages/reverie/src
